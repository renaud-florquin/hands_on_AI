# Update of zzh8829/yolov3-tf2 repository to detect keys
...
### Download repo images of keys and background
...
### Generate new images with localization by mixing keys and background

```bash
python keys_with_background.py --keys "key_wb" --background "bckgrnd" --output "keys_and_background"
```

### Convert images and CSV annotation file to TFrecord

```bash
mkdir key_db
python convert_csv_to_tfrecord.py --csv_filename keys_and_background/annotations.csv --img_dir keys_and_background --output_path key_db
```
